...
if (isset($_POST['btnAdd'])) {
       ...
       if (!empty($title) && !empty($description) && !empty($category) ) {
				...
				// upload new image
				$upload = move_uploaded_file($_FILES['product_image']['tmp_name'], 'upload/hometrending_post/' . $menu_image);

				// insert new data to menu table
				$upload_image = 'upload/hometrending_post/' . $menu_image;
            
            $sql_query = "INSERT INTO hometrending_posts (title,category_id,description,image)VALUE('$title','$category','$description','$upload_image')";
            $db->sql($sql_query);
            $result = $db->getResult();
            if (!empty($result)) {
                $result = 0;
            ...
            if ($result == 1) {
                ...
                $error['add_hometrending_posts'] = "<section class='content-header'>
                                                <span class='text text-success'>HomeTrending Post Added Successfully</span> </section>";
            } else {
                $error['add_hometrending_posts'] = " <span class='text text-danger'>Failed</span>";
            }
            }
        }
?>

    <?php echo isset($error['add_hometrending_posts']) ? $error['add_hometrending_posts'] : ''; ?>
				...
				<div class="row gutters">
					<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
						...
						<div class="card">
							...
							<form name="add_hometrending_posts_form" method="post" enctype="multipart/form-data">
								<div class="card-body">
											<h3>Add Home Trending Post</h3>
											<section>
												<h6 class="h-0 m-0">&nbsp;</h6>
												<div class="row gutters">
...
<script>
    $('#add_hometrending_posts_form').validate({
