...
if (isset($_POST['btnEdit'])) {
		if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
			...
			$upload_image = 'upload/gallery/' . $filename;

			$sql = "UPDATE homegallery SET `image`='" . $upload_image . "' WHERE `id`=" . $ID;
			$db->sql($sql);
			...
			if ($results == 1) {
				...
				$error['update_gallery'] = "<section class='content-header'>
												<span class='text text-success'>Gallery Updated Successfully</span> </section>";
			} else {
				$error['update_gallery'] = " <span class='text text-danger'>Failed</span>";
			}
			}
...
$sql_query = "SELECT * FROM homegallery WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "homegallery.php";
	</script>
<?php } ?>

    <?php echo isset($error['update_gallery']) ? $error['update_gallery'] : ''; ?>
					...
					<div class="row gutters">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							...
							<div class="card">
								...
								<form name="edit_homegallery_form" method="post" enctype="multipart/form-data">
									<div class="card-body">
