...
if (isset($_POST['btnAdd'])) {
       ...
       if (!empty($title) && !empty($description) && !empty($category) ) {
				...
				// upload new image
				$upload = move_uploaded_file($_FILES['product_image']['tmp_name'], 'upload/homefeatured_post/' . $menu_image);

				// insert new data to menu table
				$upload_image = 'upload/homefeatured_post/' . $menu_image;
            
            $sql_query = "INSERT INTO homefeatured_posts (title,category_id,description,image)VALUE('$title','$category','$description','$upload_image')";
            $db->sql($sql_query);
            $result = $db->getResult();
            if (!empty($result)) {
                $result = 0;
            ...
            if ($result == 1) {
                ...
                $error['add_homefeatured_post'] = "<section class='content-header'>
                                                <span class='text text-success'>Home Featured Post Added Successfully</span> </section>";
            } else {
                $error['add_homefeatured_post'] = " <span class='text text-danger'>Failed</span>";
            }
            }
        }
?>

    <?php echo isset($error['add_homefeatured_post']) ? $error['add_homefeatured_post'] : ''; ?>
					...
					<div class="row gutters">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							...
							<div class="card">
								...
								<form name="add_homefeatured_posts_form" method="post" enctype="multipart/form-data">
									<div class="card-body">
											<h3>Add Home Featured Post</h3>
											<section>
												<h6 class="h-0 m-0">&nbsp;</h6>
												<div class="row gutters">
...
<script>
    $('#add_homefeatured_posts_form').validate({
