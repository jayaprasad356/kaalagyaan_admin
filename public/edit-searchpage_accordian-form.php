...
if (isset($_POST['btnEdit'])) {
	...
	$area_name = $db->escapeString(($_POST['area_name']));
   ...
   if (!empty($city) && !empty($description) && !empty($category) && !empty($area_name) ) {
			if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
				...
				$upload_image = 'upload/accordians/' . $filename;
				$sql = "UPDATE searchpage_accordians SET `image`='" . $upload_image . "' WHERE `id`=" . $ID;
				$db->sql($sql);
			}
				
				$sql_query = "UPDATE searchpage_accordians SET city_id='$city',category_id='$category',description='$description',area_name='$area_name' WHERE id=$ID";
				$db->sql($sql_query);
				$result = $db->getResult();
				if (!empty($result)) {
					$result = 0;
				...
				if ($result == 1) {
					...
					$error['update_accordians'] = "<section class='content-header'>
													<span class='text text-success'>Search Page Accordians Updated Successfully</span> </section>";
				} else {
					$error['update_accordians'] = " <span class='text text-danger'>Failed</span>";
				}
				}
			}
...
$sql_query = "SELECT * FROM searchpage_accordians WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "searchpage_accordians.php";
	</script>
<?php } ?>

    <?php echo isset($error['update_accordians']) ? $error['update_accordians'] : ''; ?>
					...
					<div class="row gutters">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							...
							<div class="card">
								...
								<form name="edit_accodian_form" method="post" enctype="multipart/form-data">
									<div class="card-body">
									<input type="hidden" id="old_image" name="old_image"  value="<?= $res[0]['image']; ?>">
											...
											<section>
												...
												<div class="row gutters">
													...
													<h4>Specific images and information related to that particular place</h4>
													<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 mt-3">
														
														<div class="field-wrapper">
															<input type="text" placeholder="Enter Name" name="area_name" value="<?php echo $res[0]['area_name']; ?>">
