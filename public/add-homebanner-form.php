...
if (isset($_POST['btnAdd'])) {
       ...
       if (!empty($title) && !empty($description) && !empty($category) ) {
				...
				// upload new image
				$upload = move_uploaded_file($_FILES['product_image']['tmp_name'], 'upload/homebanner/' . $menu_image);

				// insert new data to menu table
				$upload_image = 'upload/homebanner/' . $menu_image;
            
            $sql_query = "INSERT INTO homebanners (title,category_id,description,image)VALUE('$title','$category','$description','$upload_image')";
            $db->sql($sql_query);
            $result = $db->getResult();
            if (!empty($result)) {
                $result = 0;
            ...
            if ($result == 1) {
                ...
                $error['add_homebanner'] = "<section class='content-header'>
                                                <span class='text text-success'>Homebanner Added Successfully</span> </section>";
            } else {
...
?>
    ...
    <?php echo isset($error['add_homebanner']) ? $error['add_homebanner'] : ''; ?>
