	...
	if (empty($category)) {
		...
		if (!empty($title) && !empty($description) && !empty($category) ) 
		{
			if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
				...
				$upload_image = 'upload/homebanner/' . $filename;
				$sql = "UPDATE homebanners SET `image`='" . $upload_image . "' WHERE `id`=" . $ID;
				$db->sql($sql);
			}
				
				$sql_query = "UPDATE homebanners SET title='$title',category_id='$category',description='$description' WHERE id=$ID";
				$db->sql($sql_query);
				$result = $db->getResult();
				if (!empty($result)) {
					$result = 0;
				...
				if ($result == 1) {
					...
					$error['update_homebanner'] = "<section class='content-header'>
													<span class='text text-success'>Homebanner Updated Successfully</span> </section>";
				} else {
					$error['update_homebanner'] = " <span class='text text-danger'>Failed</span>";
				}
				}
			}
...
$sql_query = "SELECT * FROM homebanners WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "homebanners.php";
	</script>
<?php } ?>

    <?php echo isset($error['update_homebanner']) ? $error['update_homebanner'] : ''; ?>
