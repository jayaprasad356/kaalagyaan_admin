	...
	if (empty($category)) {
		...
		if (!empty($title) && !empty($description) && !empty($category) ) 
		{
			if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
				...
				$target_path = 'upload/hometrending_post/';
				...
				$upload_image = 'upload/hometrending_post/' . $filename;
				$sql = "UPDATE hometrending_posts SET `image`='" . $upload_image . "' WHERE `id`=" . $ID;
				$db->sql($sql);
			}
				
				$sql_query = "UPDATE hometrending_posts SET title='$title',category_id='$category',description='$description' WHERE id=$ID";
				$db->sql($sql_query);
				$result = $db->getResult();
				if (!empty($result)) {
					$result = 0;
				...
				if ($result == 1) {
					...
					$error['update_hometrending_posts'] = "<section class='content-header'>
													<span class='text text-success'>HomeTrending Posts Updated Successfully</span> </section>";
				} else {
					$error['update_hometrending_posts'] = " <span class='text text-danger'>Failed</span>";
				}
				}
			}
...
$sql_query = "SELECT * FROM hometrending_posts WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "hometrending_posts.php";
	</script>
<?php } ?>

    <?php echo isset($error['update_hometrending_posts']) ? $error['update_hometrending_posts'] : ''; ?>
					...
					<div class="row gutters">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							...
							<div class="card">
								...
								<form name="add_homebanner_form" method="post" enctype="multipart/form-data">
									<div class="card-body">
