	...
	if (empty($category)) {
		...
		if (!empty($title) && !empty($description) && !empty($category) ) 
		{
			if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
				...
				$target_path = 'upload/homefeatured_post/';
				...
				$upload_image = 'upload/homefeatured_post/' . $filename;
				$sql = "UPDATE homefeatured_posts SET `image`='" . $upload_image . "' WHERE `id`=" . $ID;
				$db->sql($sql);
			}
				
				$sql_query = "UPDATE homefeatured_posts SET title='$title',category_id='$category',description='$description' WHERE id=$ID";
				$db->sql($sql_query);
				$result = $db->getResult();
				if (!empty($result)) {
					$result = 0;
				...
				if ($result == 1) {
					...
					$error['update_homefeatured_post'] = "<section class='content-header'>
													<span class='text text-success'>Home Featured Posts Updated Successfully</span> </section>";
				} else {
					$error['update_homefeatured_post'] = " <span class='text text-danger'>Failed</span>";
				}
				}
			}
...
$sql_query = "SELECT * FROM homefeatured_posts WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "homefeatured_posts.php";
	</script>
<?php } ?>

    <?php echo isset($error['update_homefeatured_post']) ? $error['update_homefeatured_post'] : ''; ?>
					...
					<div class="row gutters">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							...
							<div class="card">
								...
								<form name="edit_homefeatured_post_form" method="post" enctype="multipart/form-data">
									<div class="card-body">
									<input type="hidden" id="old_image" name="old_image"  value="<?= $res[0]['image']; ?>">
											<h3>Edit Home Featured Post</h3>
											<section>
												<h6 class="h-0 m-0">&nbsp;</h6>
												<div class="row gutters">
