...
if (isset($_POST['btnAdd'])) {
		  ...
		  $extension = end(explode(".", $_FILES["product_image"]["name"]));
				...
				// upload new image
				$upload = move_uploaded_file($_FILES['product_image']['tmp_name'], 'upload/gallery/' . $menu_image);

				// insert new data to menu table
				$upload_image = 'upload/gallery/' . $menu_image;
            
            $sql_query = "INSERT INTO homegallery (image)VALUE('$upload_image')";
            $db->sql($sql_query);
            $result = $db->getResult();
            if (!empty($result)) {
                $result = 0;
            ...
            if ($result == 1) {
                ...
                $error['add_homegallery'] = "<section class='content-header'>
                                                <span class='text text-success'>Gallery Added Successfully</span> </section>";
            } else {
                $error['add_homegallery'] = " <span class='text text-danger'>Failed</span>";
            }
            }
?>

    <?php echo isset($error['add_homegallery']) ? $error['add_homegallery'] : ''; ?>
					...
					<div class="row gutters">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							...
							<div class="card">
								...
								<form name="add_homegallery_form" method="post" enctype="multipart/form-data">
									<div class="card-body">
											<h3>Add Gallery</h3>
											<section>
												<h6 class="h-0 m-0">&nbsp;</h6>
