...
if (isset($_POST['btnAdd'])) {
       ...
       if (!empty($title) && !empty($description) && !empty($category) ) {
				...
				// upload new image
				$upload = move_uploaded_file($_FILES['product_image']['tmp_name'], 'upload/searchbanner/' . $menu_image);

				// insert new data to menu table
				$upload_image = 'upload/searchbanner/' . $menu_image;
            
            $sql_query = "INSERT INTO searchpage_banners (title,category_id,description,image)VALUE('$title','$category','$description','$upload_image')";
            $db->sql($sql_query);
            $result = $db->getResult();
            if (!empty($result)) {
                $result = 0;
            ...
            if ($result == 1) {
                ...
                $error['add_searchbanner'] = "<section class='content-header'>
                                                <span class='text text-success'>Banner Added Successfully</span> </section>";
            } else {
                $error['add_searchbanner'] = " <span class='text text-danger'>Failed</span>";
            }
            }
        }
?>

    <?php echo isset($error['add_searchbanner']) ? $error['add_searchbanner'] : ''; ?>
					...
					<div class="row gutters">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							...
							<div class="card">
								...
								<form name="add_searchbanner_form" method="post" enctype="multipart/form-data">
									<div class="card-body">
											<h3>Add Banner</h3>
											<section>
												<h6 class="h-0 m-0">&nbsp;</h6>
...
<script>
    $('#add_searchbanner_form').validate({
