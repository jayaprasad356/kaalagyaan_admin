	...
	if (empty($category)) {
		...
		if (!empty($title) && !empty($description) && !empty($category) ) 
		{
			if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
				...
				$upload_image = 'upload/about/' . $filename;
				$sql = "UPDATE about SET `image`='" . $upload_image . "' WHERE `id`=" . $ID;
				$db->sql($sql);
			}
				
				$sql_query = "UPDATE about SET title='$title',category_id='$category',description='$description' WHERE id=$ID";
				$db->sql($sql_query);
				$result = $db->getResult();
				if (!empty($result)) {
					$result = 0;
				...
				if ($result == 1) {
					...
					$error['update_about'] = "<section class='content-header'>
													<span class='text text-success'>About Informations Updated Successfully</span> </section>";
				} else {
					$error['update_about'] = " <span class='text text-danger'>Failed</span>";
				}
				}
			}
...
$sql_query = "SELECT * FROM about WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();
...
<?php } ?>
    ...
    <?php echo isset($error['update_about']) ? $error['update_about'] : ''; ?>
					...
					<div class="row gutters">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							...
							<div class="card">
								...
								<form name="edit_about_form" method="post" enctype="multipart/form-data">
									<div class="card-body">
